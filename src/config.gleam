...
/// Team template — defines a reusable agent team configuration via TOML.
pub type TeamTemplateConfig {
  TeamTemplateConfig(
    name: String,
    description: String,
...
pub type AppConfig {
  AppConfig(
    ...
    pm_max_tokens: Option(Int),
    pm_max_turns: Option(Int),
    pm_max_errors: Option(Int),
    ...
    // ── Teams ──
    team_max_members: Option(Int),
    team_token_budget: Option(Int),
    team_max_debate_rounds: Option(Int),
    team_templates: Option(List(TeamTemplateConfig)),
    // ── Agent turn cap ──
    max_agent_turns_cap: Option(Int),
    // ── Escalation ──
    ...
    forecaster_features_path: Option(String),
    // ── Normative calculus ──
    ...
    // ── Appraisal ──
    appraiser_model: Option(String),
    appraiser_max_tokens: Option(Int),
    appraisal_min_complexity: Option(String),
    appraisal_min_steps: Option(Int),
    // ── Affect ──
    ...
    // ── Meta-Learning: BEAM workers (post–Phase F) ──
    /// Enable the meta-learning worker pool. When True, seven BEAM
    /// workers run off-cog: three mechanical audits (affect correlation,
    /// fabrication audit, voice drift) and four judgement jobs that
    /// dispatch to the Remembrancer agent (consolidation, goal review,
    /// skill decay, strategy review). None of them route through the
    /// operator-visible scheduler. Default: True.
    meta_learning_enabled: Option(Bool),
...
pub fn default() -> AppConfig {
  AppConfig(
    ...
    appraiser_max_tokens: None,
    appraisal_min_complexity: None,
...
pub fn merge(base: AppConfig, override override_cfg: AppConfig) -> AppConfig {
  AppConfig(
    ...
    thinking_budget_tokens: option.or(
      override_cfg.thinking_budget_tokens,
    ...
    pm_max_tokens: option.or(override_cfg.pm_max_tokens, base.pm_max_tokens),
    pm_max_turns: option.or(override_cfg.pm_max_turns, base.pm_max_turns),
    pm_max_errors: option.or(override_cfg.pm_max_errors, base.pm_max_errors),
    ...
    team_max_members: option.or(
      override_cfg.team_max_members,
      base.team_max_members,
    ),
    team_token_budget: option.or(
      override_cfg.team_token_budget,
      base.team_token_budget,
    ),
    team_max_debate_rounds: option.or(
      override_cfg.team_max_debate_rounds,
      base.team_max_debate_rounds,
    ),
    team_templates: option.or(override_cfg.team_templates, base.team_templates),
    max_agent_turns_cap: option.or(
      override_cfg.max_agent_turns_cap,
      base.max_agent_turns_cap,
    ),
    ...
    forecaster_features_path: option.or(
      override_cfg.forecaster_features_path,
      base.forecaster_features_path,
    ),
    ...
    // Appraisal
    appraiser_model: option.or(
      override_cfg.appraiser_model,
      base.appraiser_model,
    ),
    appraiser_max_tokens: option.or(
      override_cfg.appraiser_max_tokens,
      base.appraiser_max_tokens,
    ),
    appraisal_min_complexity: option.or(
      override_cfg.appraisal_min_complexity,
      base.appraisal_min_complexity,
    ),
    appraisal_min_steps: option.or(
      override_cfg.appraisal_min_steps,
      base.appraisal_min_steps,
    ),
    // Affect
    affect_enabled: option.or(override_cfg.affect_enabled, base.affect_enabled),
    affect_history_window: option.or(
    ...
    meta_learning_enabled: option.or(
      override_cfg.meta_learning_enabled,
      base.meta_learning_enabled,
    ),
...
pub fn to_string(cfg: AppConfig) -> String {
  ...
  [
    ...
    option.map(cfg.thinking_budget_tokens, fn(v) {
      "thinking_budget_tokens: " <> int.to_string(v)
    }),
    // Loop control
    ...
    // Appraisal
    option.map(cfg.appraiser_model, fn(v) { "appraiser_model: " <> v }),
    option.map(cfg.appraiser_max_tokens, fn(v) {
      "appraiser_max_tokens: " <> int.to_string(v)
    }),
    option.map(cfg.appraisal_min_complexity, fn(v) {
      "appraisal_min_complexity: " <> v
    }),
    option.map(cfg.appraisal_min_steps, fn(v) {
      "appraisal_min_steps: " <> int.to_string(v)
    }),
    // Affect
...
fn toml_to_config(table: dict.Dict(String, tom.Toml)) -> AppConfig {
  ...
  AppConfig(
    ...
    thinking_budget_tokens: option.or(
      get_toml_int(table, ["anthropic", "thinking_budget_tokens"]),
      get_int("thinking_budget_tokens"),
    ),
    ...
    pm_max_tokens: get_toml_int(table, [
      "agents", "project_manager", "max_tokens",
    ]),
    pm_max_turns: get_toml_int(table, ["agents", "project_manager", "max_turns"]),
    pm_max_errors: get_toml_int(table, [
      "agents", "project_manager", "max_errors",
    ]),
    ...
    team_max_members: get_toml_int(table, ["delegation", "team_max_members"]),
    team_token_budget: get_toml_int(table, ["delegation", "team_token_budget"]),
    team_max_debate_rounds: get_toml_int(table, [
      "delegation", "team_max_debate_rounds",
    ]),
    team_templates: parse_team_templates(table),
    max_agent_turns_cap: get_toml_int(table, ["agents", "max_turns_cap"]),
    // ── [vertex] ──
    ...
    forecaster_features_path: get_toml_str(table, [
      "forecaster", "features_path",
    ]),
    ...
    // ── [appraisal] ──
    appraiser_model: get_toml_str(table, ["appraisal", "model"]),
    appraiser_max_tokens: get_toml_int(table, ["appraisal", "max_tokens"]),
    appraisal_min_complexity: get_toml_str(table, [
      "appraisal", "min_complexity",
    ]),
    appraisal_min_steps: get_toml_int(table, ["appraisal", "min_steps"]),
    // ── [affect] ──
    ...
    // ── [meta_learning] — BEAM workers (post–Phase F) ──
    meta_learning_enabled: get_toml_bool(table, ["meta_learning", "enabled"]),
...
/// Parse [teams.*] sections from TOML into TeamTemplateConfig values.
/// Each sub-table under [teams] defines a team:
///   [teams.research_review]
///   description = "..."
///   members = [["agent_name", "role", "perspective"], ...]
///   strategy = "parallel_merge"
///   # synthesis_model = "claude-sonnet-4-6"
fn parse_team_templates(
  table: dict.Dict(String, tom.Toml),
) -> Option(List(TeamTemplateConfig)) {
  case tom.get_table(table, ["teams"]) {
    Error(_) -> None
    Ok(teams_table) -> {
      let templates =
        dict.to_list(teams_table)
        |> list.filter_map(fn(entry) {
          let #(name, value) = entry
          case value {
            tom.Table(team) | tom.InlineTable(team) ->
              parse_one_team(name, team)
            _ -> Error(Nil)
          }
        })
      case templates {
...
fn parse_one_team(
  name: String,
  team: dict.Dict(String, tom.Toml),
) -> Result(TeamTemplateConfig, Nil) {
  let description = case dict.get(team, "description") {
    Ok(tom.String(s)) -> s
    _ -> ""
  }
  let strategy = case dict.get(team, "strategy") {
    Ok(tom.String(s)) -> s
    _ -> "parallel_merge"
  }
  let synthesis_model = case dict.get(team, "synthesis_model") {
    Ok(tom.String(s)) -> Some(s)
    _ -> None
  }
  let members = case dict.get(team, "members") {
    Ok(tom.Array(items)) ->
      list.filter_map(items, fn(item) {
        case item {
          tom.Array([tom.String(agent), tom.String(role), tom.String(persp)]) ->
            Ok(#(agent, role, persp))
          _ -> Error(Nil)
        }
      })
...
fn validate_toml_keys(table: dict.Dict(String, tom.Toml)) -> Nil {
  ...
  case tom.get_table(table, ["appraisal"]) {
    Ok(appraisal_table) ->
      dict.keys(appraisal_table)
      |> list.each(fn(key) {
        case list.contains(known_appraisal_keys, key) {
          True -> Nil
          False ->
            slog.warn(
              "config",
              "validate",
              "Unknown appraisal config key: \"" <> key <> "\" — possible typo?",
              None,
            )
        }
      })
  ...
  case tom.get_table(table, ["affect"]) {
    Ok(affect_table) ->
      dict.keys(affect_table)
      |> list.each(fn(key) {
        case list.contains(known_affect_keys, key) {
          True -> Nil
          False ->
            slog.warn(
              "config",
              "validate",
              "Unknown affect config key: \"" <> key <> "\" — possible typo?",
              None,
            )
        }
      })
...
fn validate_config_values(cfg: AppConfig) -> Nil {
  ...
  validate_positive("thinking_budget_tokens", cfg.thinking_budget_tokens)
