...
pub fn cautious_forecasts(features: List(Feature)) -> List(Forecast) {
  // Default to 0 (no issue) when scoring is unavailable.
  // Previous default of 1-2 caused false rejections when XStructor
  // failed — the fallback assumed problems existed without evidence.
  // Fail-open: if we can't score, we don't block.
  list.map(features, fn(f) {
    Forecast(
      feature_name: f.name,
      magnitude: 0,
      rationale: "scoring unavailable — defaulting to no issue",
    )
  })
}
