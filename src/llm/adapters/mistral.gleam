//// Mistral AI provider adapter with direct HTTP calls via httpc.
////
//// Uses the OpenAI-compatible chat completions endpoint at api.mistral.ai
//// with full tool call support. Built self-contained because gllm requires
//// an OpenRouter-specific "provider" field that Mistral doesn't return.
...
pub const mistral_base_url = "https://api.mistral.ai/v1"

pub const mistral_large = "mistral-large-latest"

pub const mistral_small = "mistral-small-latest"
...
pub fn provider(api_key: String) -> Provider {
  Provider(name: "mistral", chat: fn(req) { do_chat(api_key, req) })
}

/// Create a Mistral provider by reading MISTRAL_API_KEY from the environment.
pub fn provider_from_env() -> Result(Provider, types.LlmError) {
  case get_env("MISTRAL_API_KEY") {
    Error(Nil) -> Error(types.ConfigError(reason: "MISTRAL_API_KEY is not set"))
...
fn do_chat(
  ...
  let url = mistral_base_url <> "/chat/completions"
  let headers = [
      ...
      case status_code {
        ...
        429 ->
          Error(types.RateLimitError(
            message: "Mistral rate limit exceeded (429)",
          ))
        code if code >= 500 ->
  ...
  case json.parse(body, decoder) {
    Error(_) ->
      Error(types.DecodeError(
        reason: "Failed to decode Mistral response: " <> body,
      ))
