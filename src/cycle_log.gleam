...
import llm/types.{
  ...
  ObjectProperty, StopSequenceReached, StringProperty, TextContent,
  ThinkingContent, ToolFailure, ToolResultContent, ToolSuccess, ToolUseContent,
  ToolUseRequested, User,
}
...
pub fn log_dprime_input_evaluation(
  cycle_id: String,
  result: dprime_types.GateResult,
) -> Nil {
  let decision_str = case result.decision {
  ...
  append_entry(
    json.object([
      #("cycle_id", json.string(cycle_id)),
      #("timestamp", json.string(get_datetime())),
      #("type", json.string("dprime_input_evaluation")),
      #("decision", json.string(decision_str)),
      #("score", json.float(result.dprime_score)),
      #("layer", json.string(layer_str)),
      #("explanation", json.string(result.explanation)),
