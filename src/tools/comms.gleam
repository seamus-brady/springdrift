...
fn send_email_tool() -> Tool {
  ...
  |> tool.add_string_param(
    "in_reply_to",
    "Message ID of the email being replied to (for threading). Optional.",
    False,
  )
  |> tool.build()
}
...
fn run_send_email(
  ...
  let decoder = {
    ...
    use in_reply_to <- decode.optional_field("in_reply_to", "", decode.string)
    decode.success(#(to, subject, body, in_reply_to))
  }
  case json.parse(call.input_json, decoder) {
    ...
    Ok(#(to, subject, body, in_reply_to)) -> {
      ...
      case list.contains(allowed, normalised_to) {
        ...
        True -> {
          ...
          let recent_outbound = comms_log.count_recent_outbound(comms_dir, 1)
          case recent_outbound >= config.max_outbound_per_hour {
            True ->
              ToolFailure(
                tool_use_id: call.id,
                error: "Rate limit reached: "
                  <> int.to_string(recent_outbound)
                  <> " emails sent in the last hour (max: "
                  <> int.to_string(config.max_outbound_per_hour)
