...
pub fn hijack_probe_fail_closed_on_error_test() {
  ...
  canary.run_hijack_probe("test", "token", provider, "mock")
  |> should.be_true
}
...
pub fn leakage_probe_fail_closed_on_error_test() {
  ...
  canary.run_leakage_probe("test", "token", provider, "mock")
  |> should.be_true
}
...
pub fn run_probes_clean_when_safe_test() {
  let provider = mock.provider_with_text("SAFE")
  let result = canary.run_probes("harmless instruction", provider, "mock")
...
pub fn run_probes_fail_closed_on_error_test() {
  ...
  let result = canary.run_probes("test", provider, "mock")
