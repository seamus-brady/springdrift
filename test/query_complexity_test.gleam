...
pub fn llm_returns_simple_test() {
  ...
  query_complexity.classify("What is 2 + 2?", p, "test-model", 10_000)
  |> should.equal(Simple)
}
...
pub fn llm_returns_verbose_response_with_complex_test() {
  ...
  query_complexity.classify("Hi", p, "test-model", 10_000)
  |> should.equal(Complex)
}
...
pub fn llm_returns_unrecognised_falls_back_to_heuristic_test() {
  ...
  query_complexity.classify("Hello!", p, "test-model", 10_000)
  |> should.equal(Simple)
}

pub fn llm_error_falls_back_to_heuristic_simple_test() {
  query_complexity.classify("Hello!", err_prov(), "m", 10_000)
  |> should.equal(Simple)
}

pub fn llm_error_falls_back_to_heuristic_complex_test() {
  query_complexity.classify(string.repeat("a", 201), err_prov(), "m", 10_000)
  |> should.equal(Complex)
}
...
pub fn short_math_is_simple_test() {
  query_complexity.classify("What is 2 + 2?", err_prov(), "m", 10_000)
  |> should.equal(Simple)
}
...
pub fn long_message_over_200_chars_is_complex_test() {
  ...
  query_complexity.classify(long_query, err_prov(), "m", 10_000)
  |> should.equal(Complex)
}

pub fn exactly_200_chars_is_simple_test() {
  let boundary = string.repeat("a", 200)
  query_complexity.classify(boundary, err_prov(), "m", 10_000)
  |> should.equal(Simple)
}
...
pub fn compare_keyword_is_complex_test() {
  query_complexity.classify("Compare REST and GraphQL", err_prov(), "m", 10_000)
  |> should.equal(Complex)
}
...
pub fn analyse_uk_spelling_is_complex_test() {
  query_complexity.classify("Please analyse the data", err_prov(), "m", 10_000)
  |> should.equal(Complex)
}

pub fn design_keyword_is_complex_test() {
  query_complexity.classify("Design a database schema", err_prov(), "m", 10_000)
  |> should.equal(Complex)
}
...
pub fn in_depth_hyphen_is_complex_test() {
  query_complexity.classify(
    "Give me an in-depth explanation",
    err_prov(),
    "m",
    10_000,
  )
...
pub fn optimize_keyword_is_complex_test() {
  query_complexity.classify("Optimize this SQL query", err_prov(), "m", 10_000)
  |> should.equal(Complex)
}
...
pub fn case_insensitive_keyword_test() {
  query_complexity.classify("EXPLAIN how this works", err_prov(), "m", 10_000)
  |> should.equal(Complex)
}
...
pub fn three_question_marks_is_complex_test() {
  query_complexity.classify("Why? When? How?", err_prov(), "m", 10_000)
  |> should.equal(Complex)
}

pub fn one_question_mark_is_simple_test() {
  query_complexity.classify("What time is it?", err_prov(), "m", 10_000)
  |> should.equal(Simple)
}
