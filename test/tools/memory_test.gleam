...
pub fn memory_tools_defined_test() {
  ...
  // 14 memory tools on the cognitive loop (added kill_deputy for deputies MVP)
...
pub fn is_memory_tool_trace_moved_to_observer_test() {
  // memory_trace_fact moved to Observer agent
  memory.is_memory_tool("memory_trace_fact") |> should.be_false
}
      ...
      memory.IntrospectContext(
        ...
        teams: [
          memory.TeamEntry(
            ...
            description: "Research with review",
...
// CBR curation tools moved to Observer agent
pub fn correct_case_tool_on_observer_test() {
  let tools = memory.observer_tools()
  let names = list.map(tools, fn(t) { t.name })
  list.contains(names, "correct_case") |> should.be_true
}

pub fn annotate_case_tool_on_observer_test() {
  let tools = memory.observer_tools()
  let names = list.map(tools, fn(t) { t.name })
  list.contains(names, "annotate_case") |> should.be_true
}

pub fn suppress_case_tool_on_observer_test() {
  let tools = memory.observer_tools()
  let names = list.map(tools, fn(t) { t.name })
  list.contains(names, "suppress_case") |> should.be_true
}

pub fn boost_case_tool_on_observer_test() {
  let tools = memory.observer_tools()
  let names = list.map(tools, fn(t) { t.name })
...
// CBR curation tools moved to Observer agent
pub fn is_memory_tool_correct_case_moved_test() {
  memory.is_memory_tool("correct_case") |> should.be_false
}

pub fn is_memory_tool_annotate_case_moved_test() {
  memory.is_memory_tool("annotate_case") |> should.be_false
}

pub fn is_memory_tool_suppress_case_moved_test() {
  memory.is_memory_tool("suppress_case") |> should.be_false
}

pub fn is_memory_tool_boost_case_moved_test() {
  memory.is_memory_tool("boost_case") |> should.be_false
}
...
// review_recent and detect_patterns moved to Observer agent
pub fn review_recent_tool_on_observer_test() {
  let names = list.map(memory.observer_tools(), fn(t) { t.name })
  list.contains(names, "review_recent") |> should.be_true
}

pub fn detect_patterns_tool_on_observer_test() {
  let names = list.map(memory.observer_tools(), fn(t) { t.name })
