...
A persistent runtime for long-lived LLM agents. Integrates an auditable
execution substrate (append-only memory, supervised processes, git-backed
recovery), a case-based reasoning memory layer with hybrid retrieval, a
deterministic normative calculus for safety gating with auditable axiom
trails, and continuous ambient self-perception via a structured self-state
representation (the *sensorium*) injected each cycle without tool calls.
...
**Status:** Beta. In active development. Running in daily use. ~62,000 lines of Gleam
across 136 source files, 1,490 tests passing. Core systems (cognitive loop,
