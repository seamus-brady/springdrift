...
- **agent_remembrancer** — deep memory consolidation across months/years; reads full JSONL archive, bypasses Librarian ETS window (10 tools: deep_search, fact_archaeology, mine_patterns, resurrect_thread, consolidate_memory, restore_confidence, find_connections, write_consolidation_report, propose_skills_from_patterns, analyze_affect_performance; max 8 turns; requires `remembrancer_enabled`). Use for: historical precedent, dormant threads, pattern mining over months, consolidating a period into a report, re-verifying old facts, proposing new skills from recurring CBR patterns, surfacing affect-performance correlations (Phase D).
