...
- **agent_remembrancer** — deep memory consolidation across months/years; reads full JSONL archive, bypasses Librarian ETS window (15 tools: deep_search, fact_archaeology, mine_patterns, resurrect_thread, consolidate_memory, restore_confidence, find_connections, write_consolidation_report, propose_skills_from_patterns, analyze_affect_performance, extract_insights, promote_insight, propose_strategies_from_patterns, propose_learning_goals_from_patterns, import_legacy_strategy_facts; max 8 turns; requires `remembrancer_enabled`). Use for: historical precedent, dormant threads, pattern mining over months, consolidating a period into a report, re-verifying old facts, proposing new skills/strategies/goals from recurring CBR patterns, surfacing affect-performance correlations, extracting and promoting insights worth persisting beyond the session, one-shot migration of `strategy_pattern_*` facts from prior manual tracking into the Strategy Registry.
